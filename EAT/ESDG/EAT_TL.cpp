...
public: 
	Graph *cArray;		// Node to connection mapping
	int *src,*st;			// Query array consists of source vertices
	int noOfQueries;
	// int vCount;			// Number of Vertices present in the Original Graph
	...
	void earliestArrivalTime(int src,int rt);
...
void esdg::retriveSrcVertices(int src, int rt)
{
	int n = nCount;
	connection c;
	...
	for (int i = 0; i <= n - 1; i++)
	{
		...
		if(cArray[i].u == src && cArray[i].t>=rt)
		{
			c.u=cArray[i].u;
			c.v=cArray[i].v;
			c.t=cArray[i].t;
	...
	*/
	...
	for(int node=0; node < nCount; ++node)
    {
    	...
    	if(visited[node]==true)
		{
			...
			// aNodes++;
			bool relax=false;
			if(duration[cArray[node].v]> cArray[node].t+ cArray[node].lambda)
			{
				duration[cArray[node].v]= cArray[node].t+ cArray[node].lambda;
				relax=true;
			}
			...
			if(relax)
			{
				...
				for(int nodeIndex=vArray[node]; nodeIndex < vArray[node+1]; ++nodeIndex)
				{
					...
					// {
						...
						// if(visited[eArray[nodeIndex]]==false){
							visited[eArray[nodeIndex]]=true;
						// }
					// }
				}
			}
...
void esdg::readESDGFromFile(char* esdg, char *qFile)
{
	...
	*/
	...
	st = new int[noOfQueries];
	for(int index=0; index < noOfQueries; ++index)
		fQuery >> src[index] >> st[index];
}
...
int main(int argc, char*argv[])
{
	...
	for(int queryIndex=0; queryIndex<G.noOfQueries; ++queryIndex){
		...
		// start=clock();
		G.retriveSrcVertices(G.src[queryIndex],G.st[queryIndex]);
		G.sortSrcVertices();
		G.initialize_algo();
		start=clock();
		G.earliestArrivalTime(G.src[queryIndex],G.st[queryIndex]);
		// G.fastestPathDuration(G.src[queryIndex]);
	...
	cout<<ip_esdg<<" Run Time:"<<(total_time/G.noOfQueries)*1000<<"ms"<<endl;
